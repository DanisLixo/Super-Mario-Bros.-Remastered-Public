class_name GMLHandler extends Object
...
static func import_gml_mods() -> void:
	_clear_mods_list()
	...
	for i in DirAccess.get_files_at(mod_dir):
		...
		var json := _get_mod_manifest(i, mod_path)
		_register_mod(i, json, mod_path)

	
static func get_gml_mods(deep := false) -> Array:
	_get_disabled_mods()
	
	var dict := GML_MODS.duplicate()
	if (!deep): 
		for i in disabled_mods:
			dict.erase(i)
	return dict

static func enable_gml_mod(mod_id := "") -> void:
	if (!GML_MODS.has(mod_id)):
		return
	
	ModLoaderUserProfile.enable_mod(mod_id)

static func disable_gml_mod(mod_id := "") -> void:
	if (!GML_MODS.has(mod_id)):
		return
...
static func _clear_mods_list() -> void:
	GML_MODS.clear()
	GML_MOD_JSONS.clear()
	GML_MOD_ICONS.clear()

static func _register_mod(mod_id := "", mod_json := {}, mod_path := "") -> void:
	GML_MODS.append(mod_id)
	GML_MOD_JSONS[mod_id] = mod_json
	GML_MOD_ICONS.append(_get_mod_icon(mod_id, mod_path))

static func _get_disabled_mods() -> void:
	for mod_id in ModLoaderStore.user_profiles[ModLoaderStore.current_user_profile].mod_list:
		if (!ModLoaderStore.user_profiles[ModLoaderStore.current_user_profile].mod_list[mod_id].is_active):
			disabled_mods.append(mod_id)

static func _get_mod_manifest(mod_id := "", mod_path := "") -> Dictionary:
	var local_path := _ModLoaderPath.get_path_to_mod_manifest(mod_id)
	if (FileAccess.file_exists(local_path)):
		return JSONParser.parse_to_dict(local_path)
	...
	return JSONParser.parse_to_dict(json_str)

static func _get_mod_icon(mod_id := "", mod_path := "") -> Texture2D:
