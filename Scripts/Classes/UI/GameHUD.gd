...
func update_character_info() -> void:
	...
	%CharacterName.text = tr(CharactersHandler.CHARACTER_NAMES[int(current_chara)])
	if (CharactersHandler.CHARACTER_ICONS[int(current_chara)] != null):
		%CharacterIcon.get_node("ResourceSetterNew").json_path = CharactersHandler.CHARACTER_ICONS[int(current_chara)]
